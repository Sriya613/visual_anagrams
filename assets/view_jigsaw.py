...
class JigsawView(PermuteView):
    ...
    def inverse_view(self, noise):
        """
        Undo the jigsaw permutation applied in view().
        Works with CUDA tensors.
        """
        # Get the correct permutation size based on noise resolution
        h = noise.shape[-1]
        if h == 64:
            inv_perm = get_inv_perm(self.perm_64)
        elif h == 256:
            inv_perm = get_inv_perm(self.perm_256)
        elif h == 1024:
            inv_perm = get_inv_perm(self.perm_1024)
        else:
            raise ValueError(f"Unsupported resolution: {h}")

        # Move permutation to same device as noise (CUDA-safe)
        inv_perm = inv_perm.to(noise.device)

        # Flatten spatial dimensions and apply inverse permutation
        noise_flat = noise.flatten(1, 2)  # (C, H*W)
        noise_unperm = noise_flat[:, inv_perm]  # Apply inverse permutation
        noise_unperm = noise_unperm.view_as(noise)  # Reshape back to (C, H, W)
    ...
    def make_frame(self, im, t, knot_seed=0):
        ...
        if isinstance(im, Image.Image):
           width, height = im.size
             ...
             _, _, height, width = im.shape if im.ndim == 4 else im.size()
